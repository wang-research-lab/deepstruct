...
from model import GLMConfig, GLMModel, glm_get_params_for_weight_decay_optimization
        ...
        if model_type is not None:
            if model_type == 'multiple_choice':
                if args.cloze_eval:
                    if multi_token:
                        if args.fast_decode:
                            model = GLMForMultiTokenClozeFast(config, length_penalty=args.length_penalty)
                        else:
                            model = GLMForMultiTokenCloze(config, length_penalty=args.length_penalty)
                    else:
                        model = GLMForSingleTokenCloze(config, take_softmax=args.adapet)
                else:
                    model = GLMForSequenceClassification(config, args.hidden_size, args.output_dropout, args.pool_token,
...
def train_step(data_iterator, model, optimizer, lr_scheduler, args, timers, forward_step_func, mems=None,
    ...
    while True:
        ...
        if single_step:
            break
    if args.deepspeed:
        lm_loss_total = lm_loss_total / count if count>0 else lm_loss_total
