...
def _train(model, optimizer, lr_scheduler, forward_step,
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for iteration_, batch in enumerate(train_dataloader[0]):
            ...
            args.iteration += 1
            total_lm_loss += lm_loss if isinstance(lm_loss, float) else lm_loss.data.detach().float()


            if args.iteration % args.log_interval == 0:
                learning_rate = optimizer.param_groups[0]['lr']
                avg_lm_loss = (total_lm_loss if isinstance(total_lm_loss,float) else total_lm_loss.item()) / args.log_interval
...
if __name__ == '__main__':
    ...
    elif args.task.lower() in ['cnn_dm', 'cnn_dm_original', 'gigaword', 'blank', 'squad_generation', 'xsum', 'extraction', 'PRETRAIN']:
