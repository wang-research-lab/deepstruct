    ...
    for domain in domains:
        ...
        for slot in sorted(bstate[domain]['book'].keys()):
            if slot == 'booked':
                ...
                if bstate[domain]['book'][slot] != "":
                    booking.append(1)
                    summary_bvalue.append(["{}-book {}".format(domain, slot.strip().lower()), normalize(bstate[domain]['book'][slot].strip().lower(), False)])
                else:
                    booking.append(0)
        if domain == 'train':
        ...
        for slot in bstate[domain]['semi']:
            ...
            if bstate[domain]['semi'][slot] == 'not mentioned':
                ...
                summary_bvalue.append(["{}-{}".format(domain, slot.strip().lower()), normalize(bstate[domain]['semi'][slot].strip().lower(), False)])
            if slot_enc != [0, 0, 0]:
                domain_active = True
