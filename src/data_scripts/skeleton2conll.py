...
def pretty_print_tree_string(a_tree_string, offset=''):
    ...
    def treeindent(depth):
        return indent_string[depth_to_indent[depth]]+offset


    current_depth = 0
    for frag in  close_paren.split(a_tree):
        if frag[-1]!= '\n':
            frag=frag+')'
        else: frag=frag[0:-1]
...
if __name__ == "__main__":
    ...
    if len(sys.argv[1:]) == 2 and sys.argv[1] in ["--help", "-h"] and sys.argv[2] in transformations:
        ...
        max_key_len = max(len(t) for t in transformations) + 1
